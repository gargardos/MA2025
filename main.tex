...
Es wird nicht deutlich, wo bei den \enquote{Triggerpunkten} pro und contra liegen sollen. Soll sich dafür ausgepsrochen werden, dass Care Berufe schlechter bezahlt werden und Menschen mit Migrationsvordergrund bei Lieferando und Uber prekär beschäftigt sind? 
...
\end{myitemize}
Diese Einschränkungen haben natürlich die unangenehme Nebenwirkung, dass sie diskriminierend wirken, indem auf von der epmfundenen Norm abweichendere Leben nicht eingegangen wird. Das lässt sich zwar mit der Notwendigkeit eines didaktisch reduzierten Vorgehens begründen, ist aber nicht elegant. 
...
Die Wesbite der Arbeitsagentur, \emph{M3}, lässt sich in mehreren Hinsichten kritisieren: 
...
\begin{myenumerate}
    ...
    Eine Formel (\gls{bzw} vier) zur Berechnung ist nicht direkt aufzufinden (\gls{vgl} \gls{zb} den Erwartungshorizont). 
    Es könnte sogar das Missverständnis entstehen, der Absetzungsbetrag betrüge immer 223\,€.
    
    Erst im Downloadbereich findet sich der hundertseitige Infoflyers zum SGB II in dessen Kapitel 9.2 wird nach etwa 60 Seiten %\footnote{\url{https://www.arbeitsagentur.de/datei/merkblatt-buergergeld_ba043375.pdf} (08.07.2025)} 
    ...
    Optimalerweise wären die Zahlen des Rechenbeispiels direkt an die in den jeweiligen veröffentlichten Paragraphen veröffentlichten Zahlen gekoppelt. Wobei auch dort die Herkunft angegeben sein sollte. Da das allerdings utopisch erscheinen mag\footnote{
        \dots obwohl sich, wie im Erwartungshorizont gezeigt, auch die Bestimmungen in textlastigen Gesetzen in Formeln ausdrücken lassen. Wie an der durch die Formeln und ChatGPT erstellten \gls{abb} \ref{Absetzungsbetrag} (\gls{S} \pageref{Absetzungsbetrag}) zu sehen ist. \Gls{vgl} dazu auch die Tabelle \ref{KIHilfsmittel}, \gls{S} \pageref{KIHilfsmittel} und den eingegebenen Prompt -- es ließ sich in ChatGPT sogar der Python-Code zu der Erstellung des Plots anzeigen. Wenn es eine Sache gibt, die in der Natur von Code liegt, ist es Berechnungen durchzuführen. Es ist keine weitere Innovation notwendig, um so etwas auf Websites zu implementieren. Außerdem hätten vergleichbare Praktiken für staatliche Websites, richtig umgesetzt, neben der Qualitätsverbesserung die Chance weniger Arbeit in Zukunft zu verursachen. 
...
Wenn man sich den Graph anschaut
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{Absetzungsbetrag.png}
    \caption{Absetzungsbetrag in Abhängigkeit zum Einkommen laut den Angaben aus \emph{M3}}
...
Mindestlohnerhöhungen und von Gewerkschaften miterkämpfte Tarifabschlüsse auch für untere Einkommensschichten können den schrumpfenden Niedriglohnsektor erklären. 
